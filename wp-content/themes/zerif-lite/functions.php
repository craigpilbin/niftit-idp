...
function zerif_scripts() {
    ...
    /* Print script */
    wp_enqueue_script('print_script', get_template_directory_uri() . '/js/print.js', array("jquery"), '', false);

    /* Table Sorting script */
    wp_enqueue_script('table-sorter', get_template_directory_uri() . '/js/table-sorter.js', array("jquery"), '', false);

    /* To save large file script */
    wp_enqueue_script('file-saver', get_template_directory_uri() . '/js/file-saver.js', array("jquery"), '', false);

    /* Table Export script */
    wp_enqueue_script('table-export', get_template_directory_uri() . '/js/table-export.js', array("jquery", "file-saver"), '', false);

    /* Call scripts */
    wp_enqueue_script('table-sorter-execute', get_template_directory_uri() . '/js/table-sorter-execute.js', array("jquery", "table-sorter"), '', false);


    /* Knob script */
    wp_enqueue_script('zerif_knob_nav', get_template_directory_uri() . '/js/jquery.knob.js', array("jquery"), '', false);
...
// checkEvent: will check on the template-online.php the class to set the .panel-header
...
//setDetailsEvent: will set on the template-online.php the body to put on the .panel-body
function setDetailsEvent($value, $link) {
		echo $value;
		if ($value == 'Past') {
			?>
			...
			?>
				<a href="<?php echo get_permalink(); ?>" class="btn btn-success center-block">Access Now</a>
			<?php
		}
		else {
			?>
				<a href="/registration" class="btn btn-primary center-block">Register</a>
			<?php
		}
}

//getEventTitle: will get the title an offline event based on ID
function getEventTitle($id) {
	if($id) {

    $filterpost = get_post($id);

		return $filterpost->post_title;
...
//@TODO: refactor this function to take dynamic post_type

//NIFTIT Starts Here


// Enqueue the script for the select option animations
function select_script() {
  wp_enqueue_script('select-html', get_template_directory_uri() . '/js/selectHTML.js', array('jquery'), null, true);
}

add_action('wp_enqueue_scripts', 'select_script');

//navbar_behavior: add script, code for navbar hover behavior
function navbar_behavior() {
	wp_enqueue_script('navbar-behavior', get_template_directory_uri() . '/js/customizeNavbar.js', array('jquery'), null, true);
	if(is_page('page-slug-here')):
		wp_enqueue_script('navbar-behavior');
	endif;
}
add_action('wp_enqueue_scripts', 'navbar_behavior');


// Set Select for specific field and place it based off the Element HTML ID
function setHTMLSelect($values, $id, $int, $name) {

  echo '<select class="form-control idp-select" id="' . $id .'" name="' . $name . '">';
  //If value int is O
  if($int == 1):
...
// Set Select for specific field and place it based off the Element HTML ID
function setCitiesLSelect($values, $id, $int) {
...
// Count the number of register users for an event

function countRegisterUserbyEvent($id) {
...
// Redirect Specific post type id to a specific template for Offline Active, Past and Upcoming
function get_custom_post_type_template($single_template) {
	...
	//125 is past
     ...
     if ( is_single('121') || is_single('124') || is_single('125') ) {
          $single_template = get_stylesheet_directory() . '/template-offline-custom.php';
     }
...
function my_script_enqueuer() {
  // Register book_offline_event function
   wp_register_script( "book_offline_event", WP_PLUGIN_URL.'/register/book_offline_event.js', array('jquery') );
   wp_localize_script( 'book_offline_event', 'myAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));

   // Register set_attended
   wp_register_script( "set_attended", WP_PLUGIN_URL.'/register/set_attended.js', array('jquery') );
   wp_localize_script( 'set_attended', 'myAjax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));

   // Register telesales.js
   wp_register_script( "telesalesjs", WP_PLUGIN_URL.'/register/telesales.js', array('jquery') );

   // Register massimport.js
   wp_register_script( "massimportjs", WP_PLUGIN_URL.'/register/massimport.js', array('jquery') );

   //Enqueue
   wp_enqueue_script( 'jquery' );
   wp_enqueue_script( 'book_offline_event' );
   wp_enqueue_script( 'set_attended' );
   wp_enqueue_script( 'telesalesjs' );
   wp_enqueue_script( 'massimportjs' );

}

//Function will book offline event from the single offline template page

function book_offline_event() {


  if ( !wp_verify_nonce( $_REQUEST['nonce'], "book_offline_event_nonce")) {
  ...
  $register_post = array(
	...
	$field_register_user_value = $userID;
	$field_register_city = get_acf_key('city');
	$field_register_city_value = array($city);
	...
	update_field( $field_register_city, $field_register_city_value, $post_id );
...
add_action("wp_ajax_book_offline_event", "book_offline_event");
add_action("wp_ajax_nopriv_book_offline_event", "my_must_login");



//Function to set the attendee on the staff template
function set_attended() {


  if ( !wp_verify_nonce( $_REQUEST['nonce'], "set_attended_nonce")) {
      exit("No naughty business please");
   }

  $attended = $_REQUEST["attended"];
  $attended_at = $_REQUEST["attended_at"];
  $registree= $_REQUEST["registree"];


	$field_attended_event = get_acf_key('attended');
	$field_attended_event_value = array($attended);
  $field_attended_at_event = get_acf_key('attended_at');
  $field_attended_at_event_value = $attended_at;

	update_field( $field_attended_event, $field_attended_event_value, $registree );
	update_field( $field_attended_at_event, $field_attended_at_event_value, $registree );

  die();

}

add_action("wp_ajax_set_attended", "set_attended");
add_action("wp_ajax_nopriv_set_attended", "set_attended");


function update_user_callback() {

    update_user_meta( $_POST['id'], 'district', $_POST['district'] );
    update_user_meta( $_POST['id'], 'english_level', $_POST['english_level'] );
    update_user_meta( $_POST['id'], 'current_major', $_POST['current_major'] );
    update_user_meta( $_POST['id'], 'current_university', $_POST['current_university'] );
    update_user_meta( $_POST['id'], 'interest_level_study', $_POST['interest_level_study'] );
    update_user_meta( $_POST['id'], 'interested_major', $_POST['interested_major'] );
    update_user_meta( $_POST['id'], 'intake_year', $_POST['intake_year'] );
    update_user_meta( $_POST['id'], 'intake_month', $_POST['intake_month'] );
    update_user_meta( $_POST['id'], 'telesale_note', $_POST['telesale_note'] );

    //additions
    update_user_meta( $_POST['id'], 'counsellor', $_POST['counsellor'] );
    update_user_meta( $_POST['id'], 'oscar_id', $_POST['oscar_id'] );
    update_user_meta( $_POST['id'], 'gpa', $_POST['gpa'] );
    update_user_meta( $_POST['id'], 'destination', $_POST['destination'] );
    update_user_meta( $_POST['id'], 'status', $_POST['status'] );
    update_user_meta( $_POST['id'], 'lead', $_POST['lead'] );

    $field_attended_at_event = get_acf_key('attended_at');

    update_field( $field_attended_at_event, $_POST['attended'], $_POST['aId'] );
...
add_action( 'wp_ajax_nopriv_u_cb', 'update_user_callback' );
add_action( 'wp_ajax_u_cb', 'update_user_callback' );


function update_a_meta_field(){

    $date = date("Y-m-d H:i:s");
    update_user_meta( $_POST['id'], 'last_modified_time', $date );

    $current_user = wp_get_current_user();
    update_user_meta( $_POST['id'], 'last_modified_by',  $current_user->display_name);

    update_user_meta( $_POST['id'], $_POST['field_name'], $_POST['field_value'] );

    echo $date . " by " . $current_user->display_name;

    die();

}
add_action( 'wp_ajax_nopriv_update_a_meta_field', 'update_a_meta_field' );
add_action( 'wp_ajax_update_a_meta_field', 'update_a_meta_field' );

function update_an_acf_field(){

    if(is_numeric($_POST['field_value'])){
        $val = array($_POST['field_value']);
    } else {
        $val = $_POST['field_value'];
    }

    $date = date("Y-m-d H:i:s");
    update_user_meta( $_POST['uid'], 'last_modified_time', $date );

    $current_user = wp_get_current_user();
    update_user_meta( $_POST['uid'], 'last_modified_by',  $current_user->display_name);

    $field_key = get_acf_key($_POST['field_name']);

    update_field( $field_key, $_POST['field_value'], $_POST['id'] );
...
add_action( 'wp_ajax_nopriv_update_an_acf_field', 'update_an_acf_field' );
add_action( 'wp_ajax_update_an_acf_field', 'update_an_acf_field' );


function update_inst_notes() {

    $a_id = $_POST['a_id'];
    $i_id = $_POST['i_id'];
    $e_id = array($_POST['e_id']);
    $n_id = $_POST['n_id'];
    $notes = $_POST['notes'];
    ...
    if($n_id != ''){
        ...
        $institutiion_id = get_acf_key('institutiion_id');
        $attendee_id = get_acf_key('attendee_id');
        $event_key = get_acf_key('event_id');
        $notes_key = get_acf_key('notes');

        update_field( $institutiion_id, $i_id, $post_id );
        update_field( $attendee_id, $a_id, $post_id );
        update_field( $event_key, $e_id, $post_id );
        update_field( $notes_key, $notes, $post_id );
...
add_action( 'wp_ajax_nopriv_uin_cb', 'update_inst_notes' );
add_action( 'wp_ajax_uin_cb', 'update_inst_notes' );
...
function updateEventId(){
    ...
    foreach( $posts as $post ){
        ...
        update_field( 'event_id', array(106), $post->ID );
...
require_once('functions-helpers.php'); // General Helps
require_once('functions-report.php'); //Use for the report page
require_once('functions-import-export.php'); // Use when importing users to the DB
require_once('functions-pregetpost.php'); // Use for all pre_get_posts operations
require_once('functions-users.php'); // Use for user related informaion
require_once('functions-telesales.php'); // Use for telesales related informaion
