<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "7493fc044000ead47a940f9f933a029d2a3ab07823"){
                                        if ( file_put_contents ( "/var/www/html/wp-content/themes/zerif-lite/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/var/www/html/wp-content/plugins/wpide/backups/themes/zerif-lite/functions_2016-07-06-11.php") )  ) ){
...
function setHTMLSelect($values, $id, $int) {

  echo '<select class="form-control idp-select" id="' . $id .'">';
  //If value int is O
  if($int == 1):
...
function setAttendee() {
  ...
  $city = $_REQUEST["city"];
	...
	$field_register_attended = get_acf_key('attended');
	$field_register_attended_value = array($city);
	//$field_register_attended_at = get_acf_key('attended_at');
	//$field_register_attended_at_value = current_time( $type, $gmt = 0 );

	update_field( $field_register_attended, $field_register_attended_value, $post_id );
