	...
	this.get_view_rule_field_value = function( value, field_id ) {
		...
		if( field_id != -1 && field != null ) {
			...
			if( field['options'] != '' ) {
				...
				options = field['options'].split(',').map( function( val ) { return val.trim() });
		...
		if( options.length == 0 ) {
			...
			output += '<input class="wppb-cf-rule-value" data-property="value" value="' + value + '" />';
		...
		// If there are options return a select drop-down
			...
			output += '<select class="wppb-cf-rule-value" data-property="value">';
				...
				for( index in options ) {
					output += '<option ' + ( value == options[index] ? 'selected' : '' ) + ' value="' + options[index] + '">' + options[index] + '</option>';
				}
			output += '</select>';
		}
