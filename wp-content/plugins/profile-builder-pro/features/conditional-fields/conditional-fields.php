			...
			if( !empty( $this->wppb_manage_fields ) ){
				...
				echo '<script type="text/javascript">
						function wppbHideActions( liElement ){
							jQuery( "input[type=\'text\'], input[type=\'email\'], input[type=\'hidden\'], textarea, select option, input[type=\'checkbox\'], input[type=\'radio\']", jQuery( liElement ) ).each( function(){
					...
					if( !empty( $field["conditional-logic-enabled"] ) && !empty( $field["conditional-logic"] ) && $field["conditional-logic-enabled"] === 'yes' ){
						...
						if( !empty( $field_conditional_logic['rules'] ) ){
							foreach( $field_conditional_logic['rules'] as $rule ){
								...
								$check_fields_conditions[] = 'jQuery.inArray( "'. $rule['value'] .'", jQuery( "#wppb-form-element-'. $rule['field'] .' input[type=\"text\"], #wppb-form-element-'. $rule['field'] .' input[type=\"email\"], #wppb-form-element-'. $rule['field'] .' input[type=\"hidden\"], #wppb-form-element-'. $rule['field'] .' textarea, #wppb-form-element-'. $rule['field'] .' select option:selected, #wppb-form-element-'. $rule['field'] .' input[type=\"checkbox\"]:checked, #wppb-form-element-'. $rule['field'] .' input[type=\"radio\"]:checked" ).map(function(){return jQuery(this).val(); }).get() ) '. $operator .' -1';
							}
						}
									...
									if( '. $check_fields_in_DOM_all .' ){
										...
										jQuery( "'. $check_fields_change_all .'" ).on( "change keyup", function(){																			
											if( '. $check_fields_conditions_all .' ){
												...
												if( "show" == "'. $field_conditional_logic['action_type'] .'" ){
													if( jQuery( "#wppb-form-element-'. $field['id'] .'" ).is(":visible") )
														jQuery( "#wppb-form-element-'. $field['id'] .'" ).hide( 0, function(){ wppbHideActions(this) });
												}
												else if( "hide" == "'. $field_conditional_logic['action_type'] .'" ){
													if( ! jQuery( "#wppb-form-element-'. $field['id'] .'" ).is(":visible") )
