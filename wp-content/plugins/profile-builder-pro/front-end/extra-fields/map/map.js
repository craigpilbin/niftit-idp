jQuery( function($) {
    ...
    function render_map( $elem ) {
        ...
        // Add search box
        if( $( '#' + $elem.attr('id') + '-search-box').length > 0 ) {
     ...
     *
     */
    $(document).on( 'click', '.wppb-map-remove-marker', function(e) {
        e.preventDefault();
        remove_marker( maps[ $(this).data('map') ], $(this).data('marker') );
    });
     ...
     *
     */
    $('.wppb-user-forms').on( 'submit', function() {
        if( $( '.wppb-map-search-box:focus' ).length > 0 )
           return false;
    });
